...
const NotificationsDropdown: React.FC<NotificationsDropdownProps> = ({
  ...
  const loadNotifications = async () => {
    setLoading(true);
    try {
      const response = await authApi.get<NotificationResponse>(
  ...
  const confirmClearAll = async () => {
    try {
      ...
      setUnreadCount(0);
      if (onCountUpdate) onCountUpdate(0);
      setShowClearConfirm(false); // Hide alert after action
    } catch (error) {
    ...
    >
      ...
      <div className="flex justify-between items-center px-4 py-3 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onClose}
            className="text-gray-500 sm:hidden text-xl font-bold"
          >
            &times;
          </button>
        </div>
          ...
          notifications.map((notification) => (
            ...
            >
              <div className="flex gap-3">
                <div
                  className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-opacity-20 ${
      ...
      {showClearConfirm && (
        <Alert
          ...
          message="This will permanently delete all notifications. Are you sure?"
